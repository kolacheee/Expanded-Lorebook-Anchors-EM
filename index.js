...
function init() {
    try {
        ...
        if (typeof eventSource !== 'undefined' && event_types.WORLDINFO_ENTRIES_LOADED) {
...
function setupDragAndDrop() {
    try {
        ...
        entriesContainer.addEventListener('dragstart', (e) => {
            try {
                if (e.target && (e.target.classList.contains('world_entry') || e.target.closest('.world_entry'))) {
                    const entry = e.target.closest('.world_entry');
                    if (e.dataTransfer) {
                        e.dataTransfer.setData('text/plain', getEntryId(entry));
                        e.dataTransfer.effectAllowed = 'move';
                    }
                } else if (e.target && (e.target.classList.contains('wi-folder') || e.target.closest('.wi-folder'))) {
...
try {
    ...
    console.error('[World Info Folders] Error during initialization setup:', error);
}
